...
test_that ('works for multiphylo, standard, mcmc', {
    ...
    sm <- sfreemap.map(sfreemap.trees[1:5], sfreemap.tips, type="standard", method="mcmc", n_simulation=10)
...
test_that ('works for multiphylo, standard, mcmc, Q given', {
    ...
    sm1 <- sfreemap.map(sfreemap.trees[1:5], sfreemap.tips, type="standard", method="mcmc", n_simulation=5)
    Q <- lapply(sm1, function(x) x$Q)
    sm2 <- sfreemap.map(sfreemap.trees[1:25], sfreemap.tips, Q=Q, type="standard", method="mcmc", n_simulation=5)
