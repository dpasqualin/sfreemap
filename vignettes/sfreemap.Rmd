...
vignette: >
  ...
  %\VignetteIndexEntry{sfreemap: Simulation free stochastic mapping for Phylogenetic data.}
...
avoiding the case where a tree with higher depth showing a higher number of transitions, but
actually having the same number as a "shorter" tree, which just happen to be in a different scale.
...
```{r}
mpd <- sfreemap.map_posterior_distribution(data[[1]], data, scale.branches=TRUE, parallel=FALSE)
```
...
Let's first create the object that will hold our data to plot the correlation matrix.
The only parameter is the name of the state that will be used for the comparison.

```{r}
cor <- sfreemap.correlation('colonial')
```

Now we can run the mapping and add the results in the correlation matrix. You
can compare as many mappings as you with by just repeating the commands below. Note
that *add.data* receives the object we created above, the mapping and, as the last
paramater, the name we will in the resulting matrix to tell apart the different
...
Like we said before, it is possible to use data from different programs, as long as they
return (or you adapt) something similar as what *sfreemap.map* returns. In fact, we
based our return object on the return of the *make.simmap* function from *phytools* package,
which are using running below to compare with *sfreemap.map*.
